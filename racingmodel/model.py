...
class RacingPredictor:
    ...
    def preprocess(
        ...
        data = self._apply_base_transforms(
            df=data,
            dist_bins=self.dist_bins
        )

         # Fixing some incorrectly classified race types
        data.loc[(data['dist_furlongs'] > 22) & (data['type'] == 'Flat'), 'type'] = 'Chase'
        ...
        data = proc.get_races_in_last_n_days(
            df=data,
            windows=self.date_windows
        )

        data['hg_grp'] = proc.group_by_threshold(
            df=data,
            var='hg',
            threshold=0.1
        )
        ...
        for var in ['trainer', 'jockey']:
            data[f'{var}_grp'] = proc.group_top_n(
                df=data,
                var=var,
                num=50
            )
        ...
        data = data.astype({col: 'category' for col in data.select_dtypes(include='O').columns})
        return data
    ...
    def split_data(
        ...
        if apply_train_test_split:
            train_data = data[(data['date'] >= self.train_start_date)  & (data['date'] <= self.train_end_date)]

            test_data = data[(data['date'] > self.train_end_date)  & (data['date'] <= self.test_end_date)]
    ...
    def predict(
        ...
        return pd.Series(y_pred, index=data.index, name=f'predicted_proba_{self.target}')
    ...
    def _apply_base_transforms(
        ...
        df['flat_or_jumps'] = df['type'].apply(lambda x: 'Flat' if x == 'Flat' else 'Jumps')
