...
class RacingSimulator:
    def __init__(
        ...
        if margin < 0:
            logging.warning("Warning: 'margin' is set below 0, bets with a negative edge" \
            "may be placed.")
        ...
        for row in self.data.itertuples():
            ...
            if bet_type == 'win':
                ...
                payout = (
                    ...
                    + (stake * (win_odds * place_terms) * place_res / 2) - stake
                )
            
            self.balance += payout

            logging.debug(f"Bet placed on {row.horse} on {row.date}. Bet type: {bet_type}, Stake: {stake}, Payout: {payout}.")
    ...
    def evaluate_results(
        ...
        if self.betting_history is None:
            raise ValueError("Simulation has not been performed yet, or no bets were placed")
        
        sns.lineplot(
            data=self.betting_history,
            x='race_id',
            y='balance'
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('-rt', '--race_type', type=str, required=True, choices=['all', 'flat', 'jumps'])
    parser.add_argument('-ew', '--allow_ew_bets', action='store_true', required=False)
    parser.add_argument('-b', '--betting_strategy', type=str, choices=['fixed', 'kelly'], required=True)
    ...
    data_sim = data_sim[
        (data_sim['date'] >= sim_start_date)
        & (data_sim['date'] <= sim_end_date)
        ]
    ...
    if allow_ew_bets:
        place_model = RacingPredictor.load(
            Path(
                ...
                'artifacts',
                'models',
                f'model_place_{race_type}.joblib'
            )
        )
        ...
        if (
            (win_model.train_start_date != place_model.train_start_date)
            or (win_model.train_end_date != place_model.train_end_date)
        ):
