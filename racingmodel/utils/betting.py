...
def calculate_ev(
    ...
    if is_ew:
        if place_prob is None or place_terms is None:
            raise Exception("If 'is_ew' is True," \
            "both place_prob and place_terms must be supplied")
...
def kelly_bet(
    ...
    if win_or_ew == 'win':
        ...
        if place_prob is None or place_terms is None:
            raise ValueError("If making an 'ew' bet," \
            "place_prob and place_terms must be provided")
        ...
        return 2 * round(opt.x, 2) * balance * kelly_frac if opt.success else default_bet_amount
