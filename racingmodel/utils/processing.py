...
def get_speed_vs_avg(
    ...
    for window in windows:
        df[f'speed_vs_avg_prev_{window}'] = (
            df.groupby('horse')['speed_vs_avg']
            .transform(lambda x: x.shift(1).rolling(window=window, min_periods=1)
            .mean()
            )
        )
...
def get_field_strength(
    ...
    df['valid_race'] = df.groupby('race_id')[rating_col].transform(lambda x: x.notna().any())
    ...
    df[f'{rating_col}_diff_vs_field'] = df[rating_col] - df[f'{rating_col}_field_strength']
    df[f'{rating_col}_rank_in_race'] = df.groupby('race_id')[rating_col].rank(ascending=False, method='min')
    ...
    for window in windows:
        ...
        df[fs_col] = (
            df.groupby('horse')[f'{rating_col}_field_strength']
            .transform(lambda x: x.shift(1).rolling(window=window, min_periods=1)
            .mean()
            )
        )
        ...
        df[fs_diff_col] = (
            df.groupby('horse')[f'{rating_col}_diff_vs_field']
            .transform(lambda x: x.shift(1).rolling(window=window, min_periods=1)
            .mean()
            )
        )
        ...
        df[fs_rank_col] = (
            df.groupby('horse')[f'{rating_col}_rank_in_race']
            .transform(lambda x: x.shift(1).rolling(window=window, min_periods=1)
            .mean()
            )
        )
    ...
    for window in windows:
        ...
        for horse, group in df.groupby('horse'):
            ...
            for i in range(len(dates)):
                ...
                counts[i] = i - start  # how many races happened before the current date within the window
...
def get_win_and_place_rates(
    ...
    for window in windows:
        ...
        df[new_win_col] = (
            df.groupby(category, sort=False)[win_col]
            .transform(lambda x: x.shift(1).rolling(window=window, min_periods=1).mean())
        )

        df[new_place_col] = (
            df.groupby(category, sort=False)[place_col]
            .transform(lambda x: x.shift(1).rolling(window=window, min_periods=1).mean())
        )
...
def get_win_or_place_rate_diff(
    ...
    for res_var in result_vars:
        for comp_var in comparison_vars:
            ...
            df[f'cumul_{prefix}'] = (
                df.groupby([category_var, comp_var], observed=False)[res_var]
                .transform(lambda x: x.shift(1).cumsum())
            )

            df[f'prev_runs_{prefix}'] = (
                df.groupby([category_var, comp_var], observed=False)[res_var]
                .transform(lambda x: x.shift(1).expanding().count())
            )

            df[f'cumul_overall_{res_var}'] = (
                df.groupby(category_var)[res_var]
                .transform(lambda x: x.shift(1).cumsum())
            )

            df[f'prev_runs_overall_{res_var}'] = (
                df.groupby(category_var)[res_var]
                .transform(lambda x: x.shift(1).expanding().count())
            )
